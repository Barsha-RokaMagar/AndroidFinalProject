...
public class CardiologistPage extends AppCompatActivity {
    ...
    private String patientId = "examplePatientId"; // Replace with actual patient ID
    ...
    private void bookAppointment() {
        ...
        if (dateStr.isEmpty()) {
            Toast.makeText(CardiologistPage.this, "Please select a date", Toast.LENGTH_SHORT).show();
            return;
        }

        if (timeStr.isEmpty()) {
            Toast.makeText(CardiologistPage.this, "Please select a time", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        Model selectedDoctor = (Model) selectedRadioButton.getTag();
        String doctorId = selectedDoctor.getUsername();
        ...
        try {
            appointmentDate = sdf.parse(dateTimeStr);
        } catch (ParseException e) {
            e.printStackTrace();
            Toast.makeText(CardiologistPage.this, "Failed to parse date/time", Toast.LENGTH_SHORT).show();
            return;
        }

        Appointment appointment = new Appointment(
                null, // appointmentId will be generated by Firebase
                selectedDoctor.getName(),
                "Patient Name", // replace with actual patient name
                appointmentDate,
                timeStr,
                "Details about the appointment" // additional details if needed
        );
