...
public class GynecologistPage extends AppCompatActivity {
    ...
    private String selectedGynecologist; // Updated to Gynecologist

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_gynecologist_page); // Ensure layout file matches
        ...
        loadGynecologists(); // Load gynecologists instead of dermatologists

        doctorRadioGroup.setOnCheckedChangeListener((group, checkedId) -> {
            RadioButton selectedButton = findViewById(checkedId);
            selectedGynecologist = selectedButton != null ? selectedButton.getText().toString() : null;
            Log.d("GynecologistPage", "Selected gynecologist: " + selectedGynecologist);
        });
    }
    ...
    private void bookAppointment() {
        ...
        if (date.isEmpty() || time.isEmpty() || selectedGynecologist == null) {
        ...
        newAppointment.child("gynecologist").setValue(selectedGynecologist); // Updated to gynecologist
        newAppointment.child("patientId").setValue(patientId).addOnCompleteListener(task -> {
            if (task.isSuccessful()) {
                Toast.makeText(GynecologistPage.this, "Appointment booked", Toast.LENGTH_SHORT).show();
                ...
                intent.putExtra("specialty", "Gynecologist"); // Updated to gynecologist
                startActivity(intent);
            } else {
                Log.e("GynecologistPage", "Failed to book appointment", task.getException());
                Toast.makeText(GynecologistPage.this, "Failed to book appointment", Toast.LENGTH_SHORT).show();
            }
        });
    }

    private void goBack() {
        Intent intent = new Intent(GynecologistPage.this, PatientPage.class);
        startActivity(intent);
        finish();
    }

    private void loadGynecologists() {
        usersRef.orderByChild("specialty").equalTo("Gynecologist").addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                Log.d("GynecologistPage", "Data snapshot received: " + dataSnapshot.toString());
                doctorRadioGroup.removeAllViews();

                if (!dataSnapshot.exists()) {
                    Log.d("GynecologistPage", "No gynecologists found.");
                    Toast.makeText(GynecologistPage.this, "No gynecologists available", Toast.LENGTH_SHORT).show();
                    return;
                }
                ...
                for (DataSnapshot snapshot : dataSnapshot.getChildren()) {
                    ...
                    if (name != null) {
                        RadioButton radioButton = new RadioButton(GynecologistPage.this);
                        radioButton.setText(name);
                        doctorRadioGroup.addView(radioButton);
                        Log.d("GynecologistPage", "Added gynecologist: " + name);
                    }
                }
            }
            ...
            public void onCancelled(@NonNull DatabaseError databaseError) {
                Log.e("GynecologistPage", "Failed to load gynecologists", databaseError.toException());
                Toast.makeText(GynecologistPage.this, "Failed to load gynecologists", Toast.LENGTH_SHORT).show();
            }
        });
    }
}
